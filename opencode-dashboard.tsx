...
const listenPort = Number.isFinite(parsedPort) && parsedPort >= 0
  ? parsedPort
  : 0;
const dbPath = Deno.env.get("OPENCODE_DB") ??
...
function loadAnalytics() {
  ...
  try {
    ...
    const sessionsByProjectDay = db.prepare(`
      SELECT
    ...
    const tokensByProjectDay = db.prepare(`
      SELECT
        date(
          COALESCE(
    ...
    const requestsByProjectDay = db.prepare(`
      SELECT
        day,
        project_id,
    ...
    const usageByDayLlm = db.prepare(`
      SELECT
        date(
          COALESCE(
    ...
    const llmByDay = Object.fromEntries(
      usageByDayLlm.map((r) => [r.day, r]),
    );
    const reqByDay = Object.fromEntries(
      requestsByDay.map((r) => [r.day, r.requests]),
    );
    const usageDaysSorted = [
      ...new Set([
        ...usageByDayLlm.map((r) => r.day),
        ...requestsByDay.map((r) => r.day),
      ]),
    ].sort();
    ...
    const avgSessionsPerActiveDay = sessionDays > 0
      ? totals.sessions / sessionDays
      : 0;
    const avgSessionsPerCalendarDay = calendarSpanDays > 0
      ? totals.sessions / calendarSpanDays
      : 0;

    const avgTokensPerActiveUsageDay = activeUsageDays > 0
      ? totals.tokens_total / activeUsageDays
      : 0;
    const avgLlmResponsesPerActiveUsageDay = activeUsageDays > 0
      ? totals.llm_responses / activeUsageDays
      : 0;
    const avgRequestsPerActiveRequestDay = activeRequestDays > 0
      ? totals.requests / activeRequestDays
      : 0;

    const avgTokensPerSessionWithLlm = totals.sessions_with_llm > 0
      ? totals.tokens_total / totals.sessions_with_llm
      : 0;
    const avgLlmResponsesPerSessionWithLlm = totals.sessions_with_llm > 0
      ? totals.llm_responses / totals.sessions_with_llm
      : 0;
    const avgTokensPerLlmResponse = totals.llm_responses > 0
      ? totals.tokens_total / totals.llm_responses
      : 0;

    const perCalendarDay = calendarSpanDays > 0
      ? {
...
const dashboardCss = Deno.readTextFileSync(
  new URL("./opencode-dashboard.css", import.meta.url),
);
        ...
        <main id="main">
          ...
          <section class="reveal d2" aria-labelledby="trends-heading">
            ...
            <p class="section-lead">
              Sessions, tokens, and requests per day, stacked by project—see
              where activity concentrates (UTC calendar days).
            </p>
            <div class="charts">
          ...
          <section class="reveal d3" aria-labelledby="projects-heading">
            ...
            <p class="section-lead">
              Worktrees ranked by how often you start a session there.
            </p>
            <div id="projects"></div>
          </section>
          <footer>
            <strong>Requests</strong> = user <code>message</code> rows +{" "}
            <code>part</code> rows with <code>type: "tool"</code>.
            <strong>LLM responses</strong> = assistant <code>message</code>{" "}
            rows with{" "}
            <code>tokens.total</code>. Refresh the page or use Reload to pull
            the latest snapshot. Raw JSON lives at{" "}
            <code>/api/analytics.json</code>.
          </footer>
        </main>
