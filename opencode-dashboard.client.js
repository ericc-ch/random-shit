    ...
    function barChart(ctx, labels, values, label, color, indexAxis) {
      const ia = indexAxis || "x";
      const valueAxis = ia === "y" ? "x" : "y";
      const categoryAxis = ia === "y" ? "y" : "x";
      return new Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            [categoryAxis]: {
              ticks: { color: chartText, maxRotation: ia === "x" ? 55 : 0 },
              grid: { display: false }
            },
            [valueAxis]: {
    ...
    function dayLabelsForRange(range, sessionRows, tokenRows, requestRows) {
      const rq = requestRows ?? [];
      const combined = [
        ...sessionRows.map((r) => r.day),
        ...tokenRows.map((r) => r.day),
        ...rq.map((r) => r.day),
      ];
          ...
          plugins: {
            ...
            tooltip: {
              ...
              filter: (tooltipItem, _index, tooltipItems) => {
                ...
                const top = new Set(
                  ranked.slice(0, 5).map((it) => it.datasetIndex),
                );
    ...
    function renderTrendCharts() {
      ...
      const tr = analyticsData.series.tokensByProjectDay ?? [];
      const rr = analyticsData.series.requestsByProjectDay ?? [];
      const fs = filterRowsByRange(sr, currentRange);
      const ft = filterRowsByRange(tr, currentRange);
      const fr = filterRowsByRange(rr, currentRange);
      let labels = dayLabelsForRange(currentRange, fs, ft, fr);
      const topK = 18;
      let sessionDs = buildStackedDatasets(fs, labels, "count", topK);
      let tokenDs = buildStackedDatasets(ft, labels, "tokens", topK);
      ...
      trendCharts.push(
        stackedBarChart(
          document.getElementById("chUsage").getContext("2d"),
          labels,
          tokenDs,
          "Tokens",
        ),
    ...
    async function loadAll() {
      ...
      try {
        ...
        err.innerHTML = "<div class='error'><strong>Could not read analytics.</strong> " + e.message + "</div>";
        document.getElementById("meta").textContent = "Fix the database path or permissions, then reload.";
        return;
      }
      k.removeAttribute("aria-busy");
      k.innerHTML = "";
      document.getElementById("meta").textContent =
        "Reading " + data.meta.dbPath + " · Snapshot " + data.meta.generatedAt +
        (data.meta.firstSessionDay
          ? " · Session days " + data.meta.firstSessionDay + " → " + data.meta.lastSessionDay
          : "");
      if (data.totals.sessions === 0) {
        banner.hidden = false;
        banner.textContent =
          "No sessions in this database yet. Run OpenCode on a project, then refresh—this dashboard only reflects what is already stored locally.";
      }
      k.appendChild(kpi("Sessions", fmt(data.totals.sessions), "Recorded sessions"));
      k.appendChild(kpi("Projects", fmt(data.totals.projects), "Distinct worktrees"));
      k.appendChild(kpi("Requests", fmt(data.totals.requests), "User messages (" + fmt(data.totals.user_messages) + ") + tool parts (" + fmt(data.totals.tool_calls) + ")"));
      k.appendChild(kpi("LLM responses", fmt(data.totals.llm_responses), "Assistant rows with token totals"));
      k.appendChild(kpi("Tokens (sum)", fmt(data.totals.tokens_total), "Across counted LLM responses"));
      k.appendChild(kpi("Cost (sum)", fmt(data.totals.cost_total), "From message cost fields"));
      const spanHint =
        data.meta.calendarSpanDays > 0
          ? "Avg per day over " + data.meta.calendarSpanDays + "-day span (first → last session day)"
          : "No session span yet";
      const pc = data.averages.perCalendarDay;
      k.appendChild(kpi("Sessions/day", fmt(pc.sessions), spanHint));
      k.appendChild(kpi("Requests/day", fmt(pc.requests), spanHint));
      k.appendChild(kpi("Responses/day", fmt(pc.responses), "LLM completions with token totals · " + spanHint));
      k.appendChild(kpi("Tokens/day", fmt(pc.tokens), spanHint));
      k.appendChild(kpi("Tokens / session (LLM)", fmt(data.averages.tokensPerSessionWithLlm), "Sessions with ≥1 counted LLM response"));
      k.appendChild(kpi("LLM responses / session", fmt(data.averages.llmResponsesPerSessionWithLlm), "Average assistant completions per LLM session"));
      k.appendChild(kpi("Tokens / LLM response", fmt(data.averages.tokensPerLlmResponse), "Mean tokens per assistant completion"));

      Chart.defaults.color = chartText;
      Chart.defaults.borderColor = chartGrid;
      Chart.defaults.font.family = "'Manrope', system-ui, sans-serif";

      const mint = "rgba(94, 234, 212, 0.72)";
      const rose = "rgba(251, 113, 133, 0.65)";

      analyticsData = data;
      setRangeUI(currentRange);
      renderTrendCharts();

      const m = data.breakdowns.model.slice(0, 12);
      charts.push(barChart(
        document.getElementById("chModel").getContext("2d"),
        m.map((r) => (r.model.length > 40 ? r.model.slice(0, 38) + "…" : r.model)),
        m.map((r) => r.tokens),
        "Tokens",
        rose,
        "y"
      ...
      const p = data.breakdowns.provider.slice(0, 12);
      charts.push(barChart(
        document.getElementById("chProvider").getContext("2d"),
        p.map((r) => r.provider),
        p.map((r) => r.tokens),
        "Tokens",
        mint,
      ...
      proj.innerHTML = "";
      if (!data.breakdowns.projectSessions.length) {
        const empty = document.createElement("div");
        empty.className = "empty-state";
        empty.innerHTML = "<strong>No projects yet</strong><p>Session rows will list worktrees here once OpenCode records them.</p>";
        proj.appendChild(empty);
      } else {
        const pt = document.createElement("table");
        pt.innerHTML = "<thead><tr><th scope='col'>Worktree</th><th scope='col'>Name</th><th scope='col' class='num'>Sessions</th></tr></thead>";
        const pb = document.createElement("tbody");
        for (const r of data.breakdowns.projectSessions) {
          const tr = document.createElement("tr");
          tr.innerHTML =
            "<td>" + r.worktree + "</td><td>" + (r.name ?? "") + "</td><td class='num'>" + fmt(r.sessions) + "</td>";
          pb.appendChild(tr);
        }
        pt.appendChild(pb);
        ...
        wrap.setAttribute("aria-label", "Projects table");
        wrap.appendChild(pt);
