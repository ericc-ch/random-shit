...
function renderTrendCharts() {
  ...
  trendCharts.push(
    stackedBarChart(document.getElementById("chUsage").getContext("2d"), labels, tokenDs, "Tokens"),
  );
  trendCharts.push(
    stackedBarChart(
...
async function loadAll() {
  ...
  try {
    ...
    err.innerHTML =
      "<div class='error'><strong>Could not read analytics.</strong> " + e.message + "</div>";
    document.getElementById("meta").textContent =
      "Fix the database path or permissions, then reload.";
    return;
  }
  k.removeAttribute("aria-busy");
  k.innerHTML = "";
  ...
  k.appendChild(
    kpi(
      "Requests",
      fmt(data.totals.requests),
      "User messages (" +
        fmt(data.totals.user_messages) +
        ") + tool parts (" +
        fmt(data.totals.tool_calls) +
        ")",
    ),
  );
  k.appendChild(
    kpi("LLM responses", fmt(data.totals.llm_responses), "Assistant rows with token totals"),
  );
  ...
  k.appendChild(
    kpi("Responses/day", fmt(pc.responses), "LLM completions with token totals · " + spanHint),
  );
  k.appendChild(kpi("Tokens/day", fmt(pc.tokens), spanHint));
  k.appendChild(
    kpi(
      "Tokens / session (LLM)",
      fmt(data.averages.tokensPerSessionWithLlm),
      "Sessions with ≥1 counted LLM response",
    ),
  );
  k.appendChild(
    kpi(
      "LLM responses / session",
      fmt(data.averages.llmResponsesPerSessionWithLlm),
      "Average assistant completions per LLM session",
    ),
  );
  k.appendChild(
    kpi(
      "Tokens / LLM response",
      fmt(data.averages.tokensPerLlmResponse),
      "Mean tokens per assistant completion",
    ),
  );
  ...
  if (!data.breakdowns.projectSessions.length) {
    ...
    empty.innerHTML =
      "<strong>No projects yet</strong><p>Session rows will list worktrees here once OpenCode records them.</p>";
    proj.appendChild(empty);
  } else {
    const pt = document.createElement("table");
    pt.innerHTML =
      "<thead><tr><th scope='col'>Worktree</th><th scope='col'>Name</th><th scope='col' class='num'>Sessions</th></tr></thead>";
