    ...
    async function loadAll() {
      ...
      try {
        ...
        err.innerHTML = "<div class='error'><strong>Could not scan projects.</strong> " + e.message + "</div>";
      ...
      document.getElementById("meta").textContent =
        "Reading " + data.meta.projectsRoot + " · Snapshot " + data.meta.generatedAt +
        (data.meta.firstDay
          ? " · File days " + data.meta.firstDay + " → " + data.meta.lastDay
          : "");
      ...
      k.appendChild(kpi("Conversation dirs", fmt(data.totals.conversation_dirs), "UUID folders under agent-transcripts"));
      k.appendChild(kpi("Transcript files", fmt(data.totals.transcript_files), "JSONL logs (incl. subagents)"));
      k.appendChild(kpi("Subagent logs", fmt(data.totals.subagent_files), "JSONL under subagents/"));
      k.appendChild(kpi("JSONL lines", fmt(data.totals.jsonl_lines), "Physical newline-separated rows"));
      k.appendChild(kpi("Disk (transcripts)", fmtBytes(data.totals.bytes_total), "Sum of JSONL file sizes"));
      k.appendChild(kpi("User rows", fmt(data.totals.user_rows), "Lines with role user"));
      k.appendChild(kpi("Assistant rows", fmt(data.totals.assistant_rows), "Lines with role assistant"));
      k.appendChild(kpi("Tool uses", fmt(data.totals.tool_uses), "tool_use parts in assistant payloads"));
      k.appendChild(kpi("Requests (proxy)", fmt(data.totals.requests), "User rows + tool uses"));
      const spanHint =
        data.meta.calendarSpanDays > 0
          ? "Avg per calendar day over " + data.meta.calendarSpanDays + "-day span (first → last file day)"
      ...
      if (!data.breakdowns.projectLines.length) {
        ...
        empty.innerHTML = "<strong>No transcripts yet</strong><p>Once Cursor writes JSONL under this root, workspaces will appear here.</p>";
        proj.appendChild(empty);
      } else {
        const pt = document.createElement("table");
        pt.innerHTML = "<thead><tr><th scope='col'>Workspace slug</th><th scope='col' class='num'>Files</th><th scope='col' class='num'>Lines</th><th scope='col' class='num'>Size</th></tr></thead>";
